...
function loadStatsPage(){
	...
	for(var i = 2;i < table.rows.length;i++){
		var homescore = parseInt(table.rows[i].cells[2].innerHTML);
		var visitorscore = parseInt(table.rows[i].cells[3].innerHTML);
...
function loadPlayersPage(){
	...
	for(var i= 0;i < players.length; i++){
		text += '<a class="dropdown-item" href="#" onclick="switchPlayers(' + i + ')">' + players[i].name + '</a>';
	}
